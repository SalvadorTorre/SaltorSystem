...
export class SalidafacturaComponent implements OnInit {
        ...
        next: (resp: any) => {
          ...
          if (data) {
            this.nombreSucursalActual =
              data.nom_sucursal || 'SUCURSAL PRINCIPAL';
    ...
    this.servicioContFactura.buscarPorSucursal(Number(idSucursal)).subscribe({
      next: (resp: any) => {
        ...
        if (item) {
          ...
          const ultimoContador =
            item.contsalida !== undefined && item.contsalida !== null
              ? Number(item.contsalida)
              : 0;
  ...
  manejarTecladoModal(e: KeyboardEvent) {
    ...
    if (e.key === 'ArrowDown') {
      this.indiceChoferSeleccionado =
        (this.indiceChoferSeleccionado + 1) %
        this.listaChoferesEncontrados.length;
      ...
      if (this.listaChoferesEncontrados.length > 0) {
        this.seleccionarChofer(
          this.listaChoferesEncontrados[this.indiceChoferSeleccionado],
        );
      }
  ...
  cargarSalidaPendiente(salida: any) {
    ...
    if (salida.detsalida && salida.detsalida.length > 0) {
      ...
      const observables = salida.detsalida.map((d: any) =>
      ...
      forkJoin(observables).subscribe({
        next: (respuestas: any) => {
          ...
          listaRespuestas.forEach((resp: any) => {
            ...
            if (
              f &&
              !this.detallesSalida.some((x) => x.codFact === f.fa_codFact)
            ) {
          ...
          Swal.fire(
            'Información',
            `Se encontró una salida pendiente. Se cargaron ${this.detallesSalida.length} facturas.`,
            'info',
          );
        },
        error: (err: any) =>
          console.error('Error cargando detalles de facturas', err),
      });
    } else {
      Swal.fire(
        'Información',
        `Se encontró una salida pendiente (${this.codSalida}) sin facturas.`,
        'info',
      );
    }
  }
    ...
    this.servicioFacturacion.getByNumero(codFact).subscribe({
      next: (resp: any) => {
        ...
        if (factura.fa_envio != 2) {
          this.mostrarError(
            `La factura ${codFact} no está marcada para envío (fa_envio != 2)`,
          );
          return;
        }
        if (factura.fa_impresa !== 'S') {
          this.mostrarError(
            `La factura ${codFact} no ha sido impresa (fa_impresa != S)`,
          );
          return;
        }
  ...
  procesarGuardado() {
    if (!this.codSalida) {
      this.mostrarError(
        'No se pudo generar el código de salida. Verifique la configuración de contfactura.',
      );
      return;
    }
    ...
    const horaSalida = new Date(
      `1970-01-01T${now.toTimeString().split(' ')[0]}`,
    );
    // Calcular totales
    const canFactura = this.detallesSalida.length;
    const valFactura = this.detallesSalida.reduce(
      (sum, item) => sum + (Number(item.valFact) || 0),
      0,
    );
    ...
    peticion.subscribe({
      next: (resp: any) => {
        ...
        const actualizaciones = this.detallesSalida.map((detalle) => {
          ...
          return this.servicioFacturacion.actualizarSalidaFactura(
            detalle.codFact,
            updatePayload,
          );
        });
        ...
        if (actualizaciones.length > 0) {
          forkJoin(actualizaciones).subscribe({
            next: () => {
              Swal.fire(
                'Éxito',
                'Salida registrada y facturas actualizadas correctamente',
                'success',
              );
            ...
            error: (err) => {
              ...
              Swal.fire(
                'Atención',
                'Salida registrada pero hubo error actualizando algunas facturas',
                'warning',
              );
  ...
  actualizarContadorSalida(callback?: () => void) {
    if (
      ...
      (this.contFacturaActual.id || this.contFacturaActual.idContFact)
    ) {
      const id = this.contFacturaActual.id || this.contFacturaActual.idContFact;
      const ultimoContador =
        this.contFacturaActual.contsalida !== undefined &&
        this.contFacturaActual.contsalida !== null
          ? Number(this.contFacturaActual.contsalida)
          : 0;
      ...
      this.servicioContFactura.editarContFactura(id, payload).subscribe({
        next: () => {
          console.log(
            'Contador de salida actualizado correctamente a',
            nuevoContador,
          );
  ...
  imprimirControlSalida() {
    ...
    // Encabezado
    doc.setFontSize(10);
    doc.text(nombreSucursal, centroPagina, y, {
      align: 'center',
      maxWidth: 70,
    });
    y += 4;
    ...
    if (zonaSucursal) {
      doc.setFontSize(7);
      doc.text(zonaSucursal, centroPagina, y, {
        align: 'center',
        maxWidth: 70,
      });
      y += 4;
    ...
    doc.setFontSize(7);
    doc.text('REPORTE DE CONTROL DE SALIDA', centroPagina, y, {
      align: 'center',
    });
    y += 6;
    ...
    y += 3.5;
    doc.text(
      `Fecha: ${fmtFecha(data.fecSalida)} ${fmtHora(data.horaSalida)}`,
      margenIzquierdo,
      y,
    );
    y += 3.5;
    ...
    // Totales cabecera
    doc.text(
      `Cant. Facturas: ${data.canFact || data.CanFact}`,
      margenIzquierdo,
      y,
    );
    y += 3.5;
    doc.text(
      `Valor Total: $${fmt(data.valFact || data.ValFact)}`,
      margenIzquierdo,
      y,
    );
    y += 3.5;
    ...
    doc.text(`Total: $${fmt(data.valFact || data.ValFact)}`, 70, finalY, {
      align: 'right',
    });
    ...
    doc.setFontSize(7);
    doc.setFont('helvetica', 'bold');
    doc.text('DECLARACIÓN DE RESPONSABILIDAD', centroPagina, currentY, {
      align: 'center',
    });
    currentY += 3;
    ...
    doc.line(
      margenIzquierdo + anchoYo,
      currentY + 0.5,
      margenIzquierdo + anchoYo + anchoNombre,
      currentY + 0.5,
    ); // Subrayado
    ...
    doc.line(
      margenIzquierdo + anchoPortador,
      currentY + 0.5,
      margenIzquierdo + anchoPortador + anchoCedula,
      currentY + 0.5,
    ); // Subrayado
    doc.text(
      textoComa2,
  ...
  get totalSalida(): number {
    return this.detallesSalida.reduce(
      (sum, item) => sum + Number(item.valFact),
      0,
    );
  }
