...
-- Marca la factura como impresa por area de despacho movil:
-- - p_area = 'forjas' marca fa_impalmaf = 'S'
-- - p_area = 'hierro' marca fa_impalmap = 'S'
-- Cuando ambas areas estan impresas, marca fa_despacho = 'S'.
-- Mantiene el RPC anterior de un parametro como compatibilidad para hierro.
...
set search_path = myappdb, public
as $$
  select myappdb.mobile_marcar_factura_impalma_impresa(p_codigo, p_area);
$$;
...
set search_path = myappdb, public
as $$
  select myappdb.mobile_marcar_factura_impalma_impresa(p_codigo, 'hierro');
$$;
...
grant execute on function myappdb.mobile_marcar_factura_impalma_impresa(text, text)
to authenticated;

grant execute on function public.mobile_marcar_factura_impalma_impresa(text, text)
to authenticated;
