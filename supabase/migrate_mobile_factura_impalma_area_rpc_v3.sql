...
-- Version v3 para despacho movil.
-- - p_area = 'forjas' marca fa_impalmaf = 'S'
-- - p_area = 'hierro' marca fa_impalmap = 'S'
-- - si p_completar_otro_area = true, tambien marca el otro campo como 'S'
-- - si ambos campos quedan en 'S', marca fa_despacho = 'S'

begin;

create or replace function myappdb.mobile_marcar_factura_impalma_impresa_v3(
  p_codigo text,
  p_area text,
  p_completar_otro_area boolean default false
)
returns jsonb
language plpgsql
security definer
...
begin
  ...
  if v_area like '%forja%' then
    update myappdb.factura f
       set fa_impalmaf = 'S',
           fa_impalmap = case
             when p_completar_otro_area then 'S'
             else f.fa_impalmap
           end
     where (
...
create or replace function public.mobile_marcar_factura_impalma_impresa_v3(
  p_codigo text,
  p_area text,
  p_completar_otro_area boolean default false
)
...
set search_path = myappdb, public
as $$
  select myappdb.mobile_marcar_factura_impalma_impresa_v3(
    p_codigo,
    p_area,
    p_completar_otro_area
  );
$$;

grant execute on function myappdb.mobile_marcar_factura_impalma_impresa_v3(text, text, boolean)
to authenticated;

grant execute on function public.mobile_marcar_factura_impalma_impresa_v3(text, text, boolean)
to authenticated;
