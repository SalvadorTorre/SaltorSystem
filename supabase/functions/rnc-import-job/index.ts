...
async function runImportJob(
  ...
  try {
    ...
    if (clear.error) {
      throw new Error(`No se pudo limpiar myappdb.rnc: ${clear.error.message}`);
    }
...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    if (action === "status") {
      ...
      if (error) {
        if (isMissingRelationError(error, "rnc_import_job")) {
          return jsonResponse(200, {
            ok: false,
            message:
              "Falta la tabla myappdb.rnc_import_job. Ejecuta la migración migrate_rnc_import_job.sql.",
          });
        }
      ...
      return jsonResponse(200, {
        ok: true,
        message: data ? "Estado del job obtenido." : "No hay jobs de importación.",
        data: {
          job: toJobDto((data || null) as JobRow | null),
        },
      });
    }
